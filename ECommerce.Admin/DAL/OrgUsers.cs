...
namespace ECommerce.Admin.DAL
{
	...
	public partial class OrgUsers
	{
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(ECommerce.Admin.Model.OrgUsers model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public ECommerce.Admin.Model.OrgUsers GetModel(int UId)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select UId,EmplId,UserName,UserPwd,AddTime,Type,Status,LastLoginTime,UuserId,Upwd from OrgUsers ");
			strSql.Append(" where UId=@UId ");
			Database db = DatabaseFactory.CreateDatabase();
			DbCommand dbCommand = db.GetSqlStringCommand(strSql.ToString());
			db.AddInParameter(dbCommand, "UId", DbType.Int32,UId);
			ECommerce.Admin.Model.OrgUsers model=null;
		...
		/// 得到一个对象实体
		/// </summary>
		public ECommerce.Admin.Model.OrgUsers DataRowToModel(DataRow row)
		{
			ECommerce.Admin.Model.OrgUsers model=new ECommerce.Admin.Model.OrgUsers();
			if (row != null)
			{
				...
				if(row["UuserId"]!=null)
				{
					model.UuserId=row["UuserId"].ToString();
				}
				if(row["Upwd"]!=null)
				{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select UId,EmplId,UserName,UserPwd,AddTime,Type,Status,LastLoginTime,UuserId,Upwd ");
		...
		public DataSet GetList(int Top,string strWhere, List<SqlParameter> parameters)
		{
			...
			strSql.Append(" UId,EmplId,UserName,UserPwd,AddTime,Type,Status,LastLoginTime,UuserId,Upwd ");
			strSql.Append(" FROM OrgUsers ");
			Database db = DatabaseFactory.CreateDatabase();
			if(strWhere.Trim()!="")
			{
		...
		/// <param name="parameters">List<SqlParameter> parameters</param>
		/// </summary>
		public List<ECommerce.Admin.Model.OrgUsers> GetListArray(string strWhere, List<SqlParameter> parameters)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select UId,EmplId,UserName,UserPwd,AddTime,Type,Status,LastLoginTime,UuserId,Upwd ");
			strSql.Append(" FROM OrgUsers ");
			...
			List<ECommerce.Admin.Model.OrgUsers> list = new List<ECommerce.Admin.Model.OrgUsers>();
			using (IDataReader dataReader = db.ExecuteReader(dbCommand))
			{
		...
		/// 对象实体绑定数据
		/// </summary>
		public ECommerce.Admin.Model.OrgUsers ReaderBind(IDataReader dataReader)
		{
			ECommerce.Admin.Model.OrgUsers model=new ECommerce.Admin.Model.OrgUsers();
			object ojb; 
			ojb = dataReader["UId"];
			...
			model.UuserId=dataReader["UuserId"].ToString();
			model.Upwd=dataReader["Upwd"].ToString();
			return model;
		}
