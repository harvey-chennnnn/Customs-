...
namespace ECommerce.Web.Manage.BaselineData {
    public partial class Default : WebPage {
        ...
        protected void btnExport_Click(object sender, EventArgs e) {
            string sql = "select row_number() over(order by ComInfo.CreateDate desc,ComInfo.ID DESC) as rownum,* FROM ComInfo left join Financel on Financel.ComID=ComInfo.ComID left join CustomerService on CustomerService.ComID=ComInfo.ComID left join ProcessManu on ProcessManu.ComID=ComInfo.ComID left join DevelopmentService on DevelopmentService.ComID=ComInfo.ComID left join AnswerWrapper on AnswerWrapper.ComID=ComInfo.ComID left join DevelopAnswer on DevelopAnswer.ComID=ComInfo.ComID left join WorkAnswer on WorkAnswer.ComID=ComInfo.ComID left join ProdAnswer on ProdAnswer.ComID=ComInfo.ComID where 1=1 ";
            var name = string.Empty;
            if (!string.IsNullOrEmpty(txtRealName.Value)) {
                name = txtRealName.Value;
                sql += " and  ComName like '%" + name + "%' ";
        ...
        protected void CreateExcel(DataSet ds) {
            ...
            table.Append("<td align=\"center\"><strong>");
            table.Append("本地营业额 (CNY K)");
            table.Append("</strong></td>");
            table.Append("<td align=\"center\"><strong>");
            table.Append("出口营业额 (CNY K)");
            table.Append("</strong></td>");
            table.Append("<td align=\"center\"><strong>");
            table.Append("总营业额 (CNY K)");
            table.Append("</strong></td>");
            table.Append("<td align=\"center\"><strong>");
            table.Append("地址3");
            table.Append("</strong></td>");
