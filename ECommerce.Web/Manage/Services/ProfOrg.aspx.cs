...
namespace ECommerce.Web.Manage.Services {
    public partial class ProfOrg : UI.WebPage {
        private readonly ECommerce.Admin.DAL.ProfOrg _dataDal = new ECommerce.Admin.DAL.ProfOrg();
        ...
        private void BindData(bool isFirstPage) {
            ...
            int pageSize = 10;
            //分页查询语句
            string sql = "select row_number() over(order by  oo.CreateDate desc,oo.OID DESC) as rownum,oo.* FROM ProfOrg oo where oo.status=1 ";
            ...
            //分页方法
            Pager1.GetDataBind("Repeater", "rptListWork", sql, pageNum, pageSize, "", "rownum", "ProfOrg.aspx?name=" + name + "&");
            #endregion
        }
